...
def main():
    df = load_data()
    page = st.sidebar.selectbox("Select Option", [ "SECI Tenders" ,"Competitor Exploration",  "New Technologies", "Latest News"])
    ...
    if page == "SECI Tenders":
        ...
        for index, row in dfHdr.iterrows():
            ...
            str = str + """
            <table>
            <tr style='background-color:#82E0AA'><th>Bidder</th><th>Cap(MW)</th><th>Tariff(Rs/kWh)</th><th>Awarded Cap(MW)</th></tr>
            """
        ...
        st.header("Total Capacity in GW")
        fig = plt.figure()
        ...
        st.header("Green Capacity in GW")
        fig = plt.figure()
        ax = fig.add_axes([0,0,1,1])
        ax.axis('equal')        
        explode = (0.2,0.1, 0.1, 0.1,   0.1)
        ...
        st.header("Green Capacity in pipeline by 2023 in GW")
        fig = plt.figure()
        ax = fig.add_axes([0,0,1,1])
        ax.axis('equal')        
        explode = (0.2,0.1, 0.1, 0.1,   0.1)
        ...
        st.subheader("Total Capacity Data")        
        ...
        st.subheader("Coal Capacity Data")        
        graph = alt.Chart(dfCap).mark_bar().encode(
            x=alt.X('company_name',  title = 'Company Name') ,         
